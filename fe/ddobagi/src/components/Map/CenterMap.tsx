...
function CenterMap(){
    ...
    useEffect(() => {
        ...
        axios.get('https://j8a608.p.ssafy.io.api/api/centers/sido')
        .then(response => {
        ...
        if (selectSido) {
            // 구군 목록 가져오기
            axios
              .get(`https://j8a608.p.ssafy.io.api/api/centers/gugun?sido=${selectSido}`)
              .then((response) => {
        ...
        axios.get(`https://j8a608.p.ssafy.io.api/api/centers?sido=${selectSido}&gugun=${selectGugun}`)
            .then(response => {
  ...
  return (
    <div>
      <div className="search-box">
        <div className="row text-center align-v-center" style={{ 
          ...
          <div className="col-2 no-right-padding" style={{ margin: "20px" }}>
            <FormControl fullWidth sx={{ 
              ...
              >
                ...
                {sidos.map((sido) => (
                  // @ts-ignore
                  <MenuItem key={sido.sidoCode} value={sido.sidoCode}>{sido.sidoName}</MenuItem>
                ))}
              </Select>
            </FormControl>
          </div>
          <div className="col-2 no-right-padding" style={{ margin: "20px" }}>
            <FormControl 
              ...
              <InputLabel id="select-gugun-label" sx={{ borderRadius:"20px" }}>구/군 선택</InputLabel>
              <Select
              ...
              >
                ...
                {guguns.map((gugun) => (
                  // @ts-ignore
                  <MenuItem key={gugun.gugunCode} value={gugun.gugunCode}>{gugun.gugunName}</MenuItem>
                ))}
              </Select>
            </FormControl>
          </div>
        ...
        <div
          id="map"
          style={{ width: "100%", height: "600px", position: "relative", overflow: "hidden", borderRadius : "20px", borderStyle:"solid", borderWidth:"2px" }}
        >
        </div>
    </div>
  );
